...
// nodeIsLastAtDepth reports whether the node at pos is the last sibling
// at the given depth before the next pane boundary or shallower node.
func nodeIsLastAtDepth(nodes []ProcListNode, pos, depth int) bool {
	for j := pos + 1; j < len(nodes); j++ {
		jd := nodes[j].Depth
		if jd == 0 || jd < depth {
			return true
		}
...
// ancestorIsLastAtDepth reports whether the nearest ancestor at ancDepth
// (scanning backward from pos) is itself last at that depth.
func ancestorIsLastAtDepth(nodes []ProcListNode, pos, ancDepth int) bool {
	for j := pos - 1; j >= 0; j-- {
		if nodes[j].Depth == 0 {
			return true
		}
		if nodes[j].Depth == ancDepth {
			return nodeIsLastAtDepth(nodes, j, ancDepth)
		}
	}
	return true
}
