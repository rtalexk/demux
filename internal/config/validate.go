...
func (c Config) Validate() []ValidationIssue {
	...
	colorFields := map[string]string{
		...
		"color_state_working":    c.Theme.ColorStateWorking,
		"color_state_working_bg": c.Theme.ColorStateWorkingBg,
		"color_state_waiting":    c.Theme.ColorStateWaiting,
		"color_state_waiting_bg": c.Theme.ColorStateWaitingBg,
		"color_state_done":       c.Theme.ColorStateDone,
		"color_state_done_bg":    c.Theme.ColorStateDoneBg,
		"color_state_error":      c.Theme.ColorStateError,
		"color_state_error_bg":   c.Theme.ColorStateErrorBg,
...
func validateColorFields(fields map[string]string) []ValidationIssue {
	var issues []ValidationIssue
	for field, val := range fields {
		if val != "" && !hexColorRE.MatchString(val) {
			issues = append(issues, ValidationIssue{
				Field:   field,
				Level:   "warn",
				Message: fmt.Sprintf("expected #rrggbb hex color, got %q", val),
			})
		}
	}
	return issues
