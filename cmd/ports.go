...
func buildPortRow(pi proc.PortInfo, pidToProc map[int32]proc.Process, cwdToPane map[string]paneRef) portRow {
	p := pidToProc[pi.PID]
	session, window, pane := "—", "—", "—"
...
func runPorts(cmd *cobra.Command, _ []string) error {
	...
	for _, pi := range ports {
		rows = append(rows, buildPortRow(pi, pidToProc, cwdToPane))
	}
